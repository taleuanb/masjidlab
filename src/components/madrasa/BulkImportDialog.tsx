...
function fuzzyMatch(colName: string, aliases: string[]): boolean {
  ...
  return aliases.some((a) => { const na = normalizeStr(a); return n === na || n.includes(na) || na.includes(n); });
}
...
export function BulkImportDialog({ open, onOpenChange, orgId, onSuccess }: BulkImportDialogProps) {
  ...
  const handleFileDrop = useCallback((acceptedFiles: File[]) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!sheetName) { setFileError("Aucune feuille."); return; }
        const json = XLSX.utils.sheet_to_json<Record<string, unknown>>(wb.Sheets[sheetName], { defval: "" });
        if (!json.length) { setFileError("Fichier vide."); return; }
        const cols = Object.keys(json[0]);
        if (cols.length < 2) { setFileError("Pas assez de colonnes."); return; }
        setFileColumns(cols); setFileRows(json); setMapping(autoMap(cols));
      } catch { setFileError("Fichier invalide."); }
    };
  ...
  const sendBatches = useCallback(async (batches: ValidatedRow[][]) => {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        // Whole batch failed — pause for retry
  ...
  const handleStartImport = useCallback(() => {
    ...
    sendBatches(batches);
  }, [validatedRows, skipErrors, sendBatches]);
  ...
  const handleFinish = useCallback(() => {
    ...
    resetFileState();
    onOpenChange(false);
  }, [onSuccess, resetFileState, onOpenChange]);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (isLocked) return; if (!v) resetFileState(); onOpenChange(v); }}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto" onInteractOutside={(e) => { if (isLocked) e.preventDefault(); }}>
        ...
        {phase === "done" && importReport && (
          <div className="space-y-5 mt-2">
            <div className="text-center space-y-3 py-4">
              <PartyPopper className="h-12 w-12 text-brand-emerald mx-auto" />
              <h3 className="text-lg font-bold text-foreground">Importation terminée !</h3>
              <p className="text-sm text-muted-foreground">
                <span className="text-brand-emerald font-semibold">{importReport.successCount}</span> élève{importReport.successCount > 1 ? "s" : ""} importé{importReport.successCount > 1 ? "s" : ""} avec succès dans le Sandbox.
              </p>
            </div>

            <div className="flex flex-wrap items-center justify-center gap-2">
              <Badge variant="outline" className="text-xs bg-brand-emerald/10 text-brand-emerald border-brand-emerald/30">
                <CheckCircle2 className="h-3 w-3 mr-1" /> Succès : {importReport.successCount}
              </Badge>
              {importReport.failures.length > 0 && (
                <Badge variant="outline" className="text-xs bg-destructive/10 text-destructive border-destructive/30">
                  <AlertTriangle className="h-3 w-3 mr-1" /> Échecs : {importReport.failures.length}
                </Badge>
              )}
            </div>
            ...
            <div className="flex justify-center pt-2">
              <Button onClick={handleFinish} className="bg-brand-emerald hover:bg-brand-emerald/90">
          ...
          <div className="space-y-5 mt-2 py-4">
              ...
              {phase === "importing" ? (
                <>
                  ...
                  <p className="text-sm font-medium text-foreground">
                    Importation du lot {batchesSent + 1}/{totalBatches} en cours…
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-sm font-medium text-foreground">
                    Importation en pause — un lot a échoué
                  </p>
                </>
              )}
            </div>
            ...
            <div className="space-y-1.5 px-4">
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>{batchesSent}/{totalBatches} lot{totalBatches > 1 ? "s" : ""} envoyé{batchesSent > 1 ? "s" : ""}</span>
            ...
            {importReport && (
              <div className="flex justify-center gap-2">
                <Badge variant="outline" className="text-xs bg-brand-emerald/10 text-brand-emerald border-brand-emerald/30">
                  ✅ {importReport.successCount} réussi{importReport.successCount > 1 ? "s" : ""}
                </Badge>
                {importReport.failures.length > 0 && (
                  <Badge variant="outline" className="text-xs bg-destructive/10 text-destructive border-destructive/30">
                    ❌ {importReport.failures.length} échec{importReport.failures.length > 1 ? "s" : ""}
                  </Badge>
                )}
              </div>
            )}
            ...
            {phase === "paused" && (
              <div className="flex justify-center gap-2 pt-2">
                <Button variant="outline" size="sm" onClick={() => { setPhase("done"); }}>
                  Abandonner les lots restants
                </Button>
                <Button size="sm" onClick={handleRetryRemaining} className="bg-brand-navy hover:bg-brand-navy/90">
        ...
        {phase === "idle" && (
          <div className="space-y-5 mt-2">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <Badge variant="outline" className={hasFile ? "bg-primary/10 text-primary border-primary/30 text-xs font-semibold" : "bg-muted text-muted-foreground border-border text-xs font-semibold"}>Étape 2</Badge>
                <span className={`text-sm font-medium ${hasFile ? "text-foreground" : "text-muted-foreground"}`}>Envoi & Mapping</span>
              </div>

              {fileError && (
                <Alert variant="destructive">
                  <AlertTriangle className="h-4 w-4" /><AlertTitle>Erreur de lecture</AlertTitle>
                  <AlertDescription className="text-xs">{fileError}</AlertDescription>
                </Alert>
              )}

              {!hasFile ? (
                <div {...getRootProps()} className={`rounded-lg border-2 border-dashed p-8 text-center cursor-pointer transition-colors ${isDragActive ? "border-primary bg-primary/5" : "border-border bg-muted/20 hover:border-primary/50 hover:bg-muted/40"}`}>
                  <input {...getInputProps()} />
                  <Upload className={`h-8 w-8 mx-auto mb-3 ${isDragActive ? "text-primary" : "text-muted-foreground/40"}`} />
                  {isDragActive ? <p className="text-sm text-primary font-medium">Déposez ici…</p> : (
                    <><p className="text-sm text-muted-foreground font-medium">Glissez votre fichier ou <span className="text-primary underline">parcourez</span></p>
                    <p className="text-xs text-muted-foreground/60 mt-1">.xlsx, .csv</p></>
                  )}
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={resetFileState}><X className="h-3.5 w-3.5" /></Button>
                  </div>
                  ...
                  {!validatedRows && (
                    <>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {SYSTEM_FIELDS.filter((f) => f.group === group).map((field) => (
                                <div key={field.key} className="flex items-center gap-2">
                                  <span className="text-xs text-foreground w-28 shrink-0">{field.label}{field.required && <span className="text-destructive ml-0.5">*</span>}</span>
                                  <Select value={mapping[field.key] || "__none__"} onValueChange={(v) => updateMapping(field.key, v)}>
                                    <SelectTrigger className="h-8 text-xs flex-1"><SelectValue placeholder="— Non mappé —" /></SelectTrigger>
                                    <SelectContent>
                      ...
                      {previewRows.length > 0 && (
                        <div className="space-y-2">
                          <div className="flex items-center gap-1.5"><Table2 className="h-3.5 w-3.5 text-muted-foreground" /><p className="text-xs font-medium text-muted-foreground">Aperçu brut (3 premières lignes)</p></div>
                          <ScrollArea className="rounded-lg border border-border"><div className="overflow-x-auto">
                            <Table><TableHeader><TableRow>{fileColumns.slice(0, 8).map((col) => <TableHead key={col} className="text-[10px] whitespace-nowrap px-2 py-1.5">{col}</TableHead>)}{fileColumns.length > 8 && <TableHead className="text-[10px] px-2 py-1.5">…</TableHead>}</TableRow></TableHeader>
                            <TableBody>{previewRows.map((row, i) => <TableRow key={i}>{fileColumns.slice(0, 8).map((col) => <TableCell key={col} className="text-[11px] px-2 py-1 whitespace-nowrap max-w-[120px] truncate">{String(row[col] ?? "")}</TableCell>)}{fileColumns.length > 8 && <TableCell className="text-[11px] px-2 py-1 text-muted-foreground">…</TableCell>}</TableRow>)}</TableBody></Table>
                          </div></ScrollArea>
                        </div>
                      )}
                    </>
                  ...
                  {validatedRows && stats && (
                    <div className="space-y-3">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-xs bg-brand-emerald/10 text-brand-emerald border-brand-emerald/30"><CheckCircle2 className="h-3 w-3 mr-1" />Prêts : {stats.valid}</Badge>
                        {stats.errors > 0 && <Badge variant="outline" className="text-xs bg-destructive/10 text-destructive border-destructive/30"><AlertTriangle className="h-3 w-3 mr-1" />Erreurs : {stats.errors}</Badge>}
                      </div>
                      <ScrollArea className="rounded-lg border border-border max-h-[280px]">
                        <TooltipProvider delayDuration={200}>
                          ...
                          <TableBody>{validatedRows.map((row) => (
                            <TableRow key={row.rowIndex} className={row.status === "error" ? "bg-destructive/5" : ""}>
                              ...
                              <TableCell className="text-center px-2 py-1">
                                {row.status === "valid" ? <CheckCircle2 className="h-3.5 w-3.5 text-brand-emerald mx-auto" /> : (
                                  <Tooltip><TooltipTrigger asChild><AlertTriangle className="h-3.5 w-3.5 text-destructive mx-auto cursor-help" /></TooltipTrigger>
                                  <TooltipContent side="left" className="max-w-xs"><ul className="text-xs space-y-0.5">{row.errors.map((err, i) => <li key={i}>• {err}</li>)}</ul></TooltipContent></Tooltip>
                                )}
                              </TableCell>
                            </TableRow>
                          ))}</TableBody></Table>
                      ...
                      {stats.errors > 0 && stats.valid > 0 && (
                        <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2">
                          ...
                          <label htmlFor="skip-errors" className="text-xs text-foreground cursor-pointer">
                            Ignorer les {stats.errors} erreur(s) et importer les {stats.valid} valide(s)
                          </label>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <Button variant="outline" size="sm" onClick={resetFileState}>
                      <RefreshCw className="h-3.5 w-3.5 mr-1.5" />{validatedRows ? "Recommencer" : "Changer de fichier"}
                    </Button>
                    {validatedRows && <Button variant="outline" size="sm" onClick={() => setValidatedRows(null)}>Modifier le mapping</Button>}
                    <div className="flex-1" />
                    {!validatedRows ? (
                      <Button size="sm" onClick={handleAnalyze} disabled={!requiredMapped || analyzing} className="bg-brand-navy hover:bg-brand-navy/90">
                        {analyzing ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" /> : <CheckCircle2 className="h-3.5 w-3.5 mr-1.5" />}
                        Analyser les données
                      </Button>
                    ) : (
                      <Button size="sm" disabled={!canImport} onClick={handleStartImport} className="bg-brand-emerald hover:bg-brand-emerald/90">
