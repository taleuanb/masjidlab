...
import { useEvalCriteria, useClassStudents, useEvalGrades } from "@/hooks/useEvaluationData";
import { ArrowLeft, Save, BarChart3, Loader2, CheckCircle } from "lucide-react";
...
export function GradingGrid({ evaluation, classId, className: clsName, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    for (const s of students) {
      ...
      for (const cr of criteria) {
        // Check if there's a grade matching student + criterion (stored as comment format "criterion_id:score")
        const existing = existingGrades.find(
          (g) => g.student_id === s.id && g.comment?.startsWith(`cr:${cr.id}`)
        );
  ...
  const doSave = useCallback(async (currentGrades: typeof grades) => {
    ...
    setSaving(true);
    try {
      // Delete existing grades for this eval
      ...
      for (const [studentId, crGrades] of Object.entries(currentGrades)) {
        for (const [crId, scoreStr] of Object.entries(crGrades)) {
          ...
          rows.push({
            ...
            org_id: orgId,
            score: numScore,
            comment: `cr:${crId}`, // tag to identify criterion
          });
        }
        // Also save final score as a row without criterion tag
        const final = Object.entries(crGrades).reduce((sum, [_, v]) => {
          const n = Number(v);
          return sum + (isNaN(n) || v === "" ? 0 : n);
        }, 0);
        const hasAny = Object.values(crGrades).some((v) => v !== "");
        if (hasAny) {
          rows.push({
            evaluation_id: evaluation.id,
            student_id: studentId,
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, studentIdx: number, criterionIdx: number) => {
    const inputsPerRow = criteria.length;
    if (e.key === "Enter") {
      e.preventDefault();
      // Move down to same criterion, next student
      ...
      // Default tab moves to next criterion
    }
  };
          ...
          <div className="flex items-center gap-2 shrink-0">
            {lastSaved && (
              <span className="text-xs text-muted-foreground flex items-center gap-1">
                <CheckCircle className="h-3 w-3 text-brand-emerald" /> Sauvegardé
              </span>
            )}
            ...
            <Table>
              <TableHeader className="sticky top-0 z-10 bg-background">
                <TableRow className="bg-muted/40">
                  <TableHead className="text-xs uppercase text-muted-foreground w-[180px] sticky left-0 bg-muted/40 z-20">Élève</TableHead>
                  {criteria.map((cr) => (
            ...
            <div className="mt-6 space-y-4">
                ...
                return (
                  <div key={cr.id} className="flex items-center justify-between py-2 border-b border-border/50">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-bold">{val || "—"}<span className="text-sm font-normal text-muted-foreground">/{cr.max_score}</span></p>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <p className="text-xl font-bold text-primary">
                  {getFinalScore(sheetStudent.id)?.toFixed(1) ?? "—"}<span className="text-sm font-normal text-muted-foreground">/{totalPoints}</span>
                </p>
              </div>
            </div>
          )}
